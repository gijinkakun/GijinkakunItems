...
public class MythrilbreakerAbility {
    ...
    public static void apply(PlayerDeathEvent event, GijinkakunItems plugin) {
        ...
        if (killer != null && isSpecialItem(killer.getInventory().getItemInMainHand(), "mythrilbreaker", plugin)) {
            CustomDeathMessageHandler.handleDeathMessage(event, "mythrilbreaker", killer);
        }
    }
