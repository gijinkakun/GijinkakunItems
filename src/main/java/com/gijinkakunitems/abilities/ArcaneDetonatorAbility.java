...
public class ArcaneDetonatorAbility {
    ...
    public static void apply(ProjectileHitEvent event, GijinkakunItems plugin) {
        if (event.getEntity() instanceof Arrow) {
            ...
            if (arrow.getShooter() instanceof Player) {
                ...
                if (bow.getType() == Material.BOW && isSpecialItem(bow, "arcanedetonator", plugin)) {
    ...
    public static void apply(PlayerDeathEvent event, GijinkakunItems plugin) {
        ...
        if (killer != null && isSpecialItem(killer.getInventory().getItemInMainHand(), "arcanedetonator", plugin)) {
            CustomDeathMessageHandler.handleDeathMessage(event, "arcanedetonator", killer);
        }
    }
